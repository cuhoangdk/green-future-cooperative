...
const loadPost = async (page: number = 1) => {
    try {
        ...
        const response = await fetchPostByCategorySlug(route.params.slug as string, page, 8);
        posts.value = {
            posts: response.data,
...
<template>
    ...
    <div v-else-if="posts?.posts.length > 0 || posts?.posts === null" class="min-h-screen bg-white items-center flex flex-col mt-12 pb-5 lg:mt-0">
